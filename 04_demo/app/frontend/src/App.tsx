...
const pipelineSteps = [
  ...
  { id: 'evidence', title: 'Query evidence services', body: 'Call the live ClinVar, VEP, and SpliceAI endpoints for the demo case.' },
  { id: 'review', title: 'Prepare clinician lane', body: 'Assemble the review draft, evidence rail, and final approval actions.' },
] as const
...
type ActionMode = 'idle' | 'approving' | 'dropping' | 'downloading_pdf' | 'saving_draft'
...
const EDITOR_FIELDS: Array<{ key: keyof Omit<ReportDraftUpdatePayload, 'review_note'>; label: string }> = [
...
export default function App() {
  ...
  const [reportDraft, setReportDraft] = useState<ReportDraftUpdatePayload | null>(null)
  ...
  const activeSession =
    sessionRuns.find((item) => item.run.run_id === selectedRunId) ?? sessionRuns[0] ?? null
  const evidenceCards = activeSession ? buildEvidenceCards(activeSession.run.evidence) : []
  const activeEvidenceId =
    selectedEvidence && evidenceCards.some((card) => card.id === selectedEvidence)
      ? selectedEvidence
      : evidenceCards[0]?.id ?? null
  const selectedEvidenceCard =
    evidenceCards.find((card) => card.id === activeEvidenceId) ?? evidenceCards[0] ?? null
  ...
  const reportTitle = activeSession?.upload.extracted_case.report_title || 'Whole Exome Sequencing Report'
  ...
  const previewPdfUrl =
    activeSession ? buildRunPdfUrl(activeSession.run.run_id, previewVersion) : null
  ...
  async function handleApprove() {
    ...
    try {
      ...
      if (noteValue.trim() && activeSession.run.review_status === 'pending_review') {
  ...
  async function handleDrop() {
    ...
    try {
      const dropResult = await dropRun(activeSession.run.run_id, noteValue.trim() || undefined)
      updateSessionRun(activeSession.run.run_id, (current) => ({
        ...current,
  ...
  function openEditor() {
    ...
    setReportDraft({
      clinical_phenotype: activeSession.run.report_payload.clinical_phenotype ?? '',
      ai_clinical_summary: activeSession.run.report_payload.ai_clinical_summary ?? '',
      expanded_evidence: activeSession.run.report_payload.expanded_evidence ?? '',
      acmg_classification: activeSession.run.report_payload.acmg_classification ?? '',
      clinical_integration: activeSession.run.report_payload.clinical_integration ?? '',
      expected_symptoms: activeSession.run.report_payload.expected_symptoms ?? '',
  ...
  function updateDraftField(field: keyof ReportDraftUpdatePayload, value: string) {
  ...
  async function handleSaveDraft() {
    ...
    try {
      const updatedRun = await updateRunReportPayload(activeSession.run.run_id, reportDraft)
      ...
      setReportDraft({
        ...
        ai_clinical_summary: updatedRun.report_payload.ai_clinical_summary ?? '',
        expanded_evidence: updatedRun.report_payload.expanded_evidence ?? '',
        acmg_classification: updatedRun.report_payload.acmg_classification ?? '',
        clinical_integration: updatedRun.report_payload.clinical_integration ?? '',
  ...
  function updateSessionRun(runId: string, updater: (run: RunResponse) => RunResponse) {
  ...
  return (
    <div className="min-h-screen bg-[color:var(--canvas)] text-[color:var(--ink)]">
      <motion.header
        className="sticky top-0 z-30 border-b border-[color:var(--line)] bg-white/84 backdrop-blur-2xl"
        initial="hidden"
        animate="show"
        variants={surfaceReveal}
        custom={0}
      >
        <div className="mx-auto flex max-w-[1840px] flex-col gap-6 px-5 py-5 sm:px-8 xl:flex-row xl:items-center xl:justify-between xl:px-10 xl:py-6">
          <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:gap-10">
            <div className="text-[25px] font-semibold tracking-[-0.04em]">HSIL demo</div>

            <nav className="flex flex-wrap items-center gap-x-8 gap-y-3 text-[11px] font-semibold uppercase tracking-[0.18em] text-[color:var(--muted)] sm:text-xs">
              <motion.div
                whileHover={{ y: -1 }}
                className="border-b-[3px] border-[color:var(--teal)] pb-3 text-[color:var(--teal)]"
              >
                Batch status: active
              </motion.div>
              <motion.div
          ...
          <motion.div
            whileHover={{ y: -1 }}
            className="hidden items-center gap-3 rounded-full border border-white/80 bg-[color:var(--shell-soft)] px-4 py-2 text-[11px] font-semibold uppercase tracking-[0.16em] text-[color:var(--muted-ink)] md:inline-flex"
          >
            <Sparkles className="h-3.5 w-3.5 text-[color:var(--teal)]" />
            {activeSession ? formatReviewLabel(activeSession.run.review_status) : 'Review lane waiting'}
          </motion.div>
        </div>
      </motion.header>

      <motion.section
        className="border-b border-[color:var(--line)] bg-[color:var(--wash)]"
        initial="hidden"
        animate="show"
        variants={surfaceReveal}
        custom={0.06}
      >
        <div className="mx-auto grid max-w-[1840px] gap-8 px-5 py-7 sm:px-8 xl:grid-cols-[minmax(0,1.3fr)_minmax(320px,1fr)_230px] xl:items-center xl:gap-10 xl:px-10 xl:py-8">
          <div>
            <p className="mb-4 text-[11px] font-semibold uppercase tracking-[0.24em] text-[color:var(--muted-ink)]">
              Submit reports
            </p>
            ...
            <div className="flex flex-col gap-4">
              <div className="flex flex-wrap items-center gap-3">
                ...
                <motion.button
                  ...
                  className="inline-flex items-center gap-3 rounded-full border border-[color:var(--line-strong)] bg-white/78 px-4 py-3 text-sm font-semibold text-[color:var(--ink)] shadow-[0_10px_24px_rgba(31,47,56,0.05)] backdrop-blur-sm transition-colors hover:border-[color:var(--teal-soft)] hover:bg-white"
                >
                  <FileUp className="h-4 w-4 text-[color:var(--teal)]" strokeWidth={2.1} />
                  {pendingFile ? 'Replace PDF' : 'Choose PDF'}
                </motion.button>

                <div className="inline-flex rounded-full border border-[color:var(--line)] bg-white/62 p-1">
                  {(['test', 'patient'] as const).map((kind) => (
                    <button
                      key={kind}
                      type="button"
                      onClick={() => setReportKind(kind)}
                      className={cn(
                        'rounded-full px-3 py-2 text-[11px] font-semibold uppercase tracking-[0.16em] transition-colors',
                        reportKind === kind
                ...
                {tabsToRender.length ? (
                  <Tabs value={selectedRunId ?? tabsToRender[0].run.run_id} onValueChange={setSelectedRunId}>
                    <TabsList className="flex-wrap">
                      ...
                      {overflowCount > 0 ? (
                        <motion.div whileHover={{ y: -2 }}>
                          <TabsTrigger value={selectedRunId ?? tabsToRender[0].run.run_id} className="border-dashed text-[color:var(--muted-ink)]">
                            +{overflowCount}
                          </TabsTrigger>
                        </motion.div>
                      ) : null}
              ...
              <motion.p
                ...
                transition={{ duration: 0.22 }}
                className="text-[14px] text-[color:var(--muted-ink)]"
              >
                {pendingFile
          ...
          <div className="xl:border-l xl:border-[color:var(--line)] xl:pl-10">
            <motion.div
              whileHover={{ y: -2 }}
              className="inline-flex items-center gap-4 rounded-[20px] bg-white/82 px-6 py-5 shadow-[0_10px_30px_rgba(31,47,56,0.05)] backdrop-blur-sm"
            >
              <FlaskConical className="h-5 w-5 text-[color:var(--teal)]" strokeWidth={1.8} />
              <span className="text-[14px] font-medium text-[color:var(--ink)]">{statusMessage}</span>
            </motion.div>
          </div>
          ...
          <div className="flex justify-end">
            <Button
              size="lg"
              className="min-w-52 rounded-[18px] text-[17px] tracking-[-0.02em]"
            ...
            >
              {isWorking ? (
                ...
                <Play className="h-4 w-4 transition-transform duration-200 group-hover:translate-x-0.5" strokeWidth={2.2} />
              )}
              {pipelineMode === 'creating_run' ? 'Creating run' : isWorking ? 'Uploading report' : 'Run pipeline'}
            </Button>
          </div>
        </div>
      </motion.section>

      <div className="mx-auto max-w-[1840px] px-4 pb-6 pt-4 sm:px-6 xl:px-8 xl:pb-8 xl:pt-6">
      <motion.main
          className="overflow-hidden rounded-[34px] border border-white/70 bg-[color:var(--shell)] shadow-[0_34px_80px_rgba(28,43,52,0.08)]"
          initial="hidden"
          animate="show"
          variants={surfaceReveal}
          custom={0.12}
        >
          <div className="grid 2xl:grid-cols-[minmax(0,1fr)_470px]">
            <section className="relative px-6 py-8 sm:px-10 xl:px-14 xl:py-12">
              <div className="pointer-events-none absolute inset-x-0 top-0 h-28 bg-[radial-gradient(circle_at_top_left,rgba(44,109,112,0.08),transparent_54%)]" />

              <div className="relative mb-10 flex flex-col gap-6 xl:mb-12 xl:flex-row xl:items-start xl:justify-between">
                <div>
                  <h1 className="text-[34px] font-semibold tracking-[-0.055em] sm:text-[42px] xl:text-[56px]">
                  ...
                  <motion.p
                    ...
                    animate={{ opacity: 1, y: 0 }}
                    transition={{ duration: 0.22 }}
                    className="mt-3 text-[17px] text-[color:var(--muted-ink)] xl:text-[20px]"
                  >
                    {activeSession ? (
                      <>
                        Internal Case Reference:{' '}
                        <span className="font-medium text-[color:var(--teal)]">#{caseReference}</span>
                      </>
                    ) : (
                ...
                <div className="flex items-center gap-3 pt-0 text-[color:var(--muted-ink)] xl:pt-4">
                  <ActionIcon
                    label="Print report"
                    icon={Printer}
                    disabled={!isApproved}
              ...
              <motion.section
                layout
                className="rounded-[30px] bg-white/76 px-6 py-7 shadow-[inset_0_1px_0_rgba(255,255,255,0.92)] backdrop-blur-sm sm:px-8 xl:px-10 xl:py-10"
              >
                <div className="mb-10 flex flex-col gap-7 xl:mb-12 xl:flex-row xl:items-start xl:justify-between">
                  <div>
                    <p className="text-[11px] font-semibold uppercase tracking-[0.28em] text-[color:var(--teal)]">
                      {activeSession ? 'Genomic referral support' : 'Pipeline intake'}
                    </p>
                    <h2 className="mt-5 max-w-3xl text-[34px] leading-[1.02] font-semibold tracking-[-0.055em] sm:text-[42px] xl:text-[56px]">
                      {activeSession ? reportTitle : 'Upload a report and let the backend populate this review surface'}
                    </h2>
                  </div>
                  ...
                  <div className="text-left xl:text-right">
                    ...
                    <AnimatePresence mode="wait">
                      <motion.p
                        ...
                        className="mt-3 text-[24px] font-medium tracking-[-0.03em] xl:text-[30px]"
                      >
                        {activeSession ? reportDate : formatPipelineMode(pipelineMode)}
                      </motion.p>
                    </AnimatePresence>
                  </div>
                </div>
                ...
                {activeSession ? (
                  <>
                    <div className="grid gap-8 md:grid-cols-3 md:gap-10">
                      <Detail label="Case ID" value={formatCaseReference(activeSession.run.patient_id)} />
                      ...
                      <div className="divide-y divide-[color:var(--line)]/90">
                        <AnimatePresence mode="popLayout">
                          {variantRows.map((item, index) => (
                            ...
                            >
                              <motion.div
                                whileHover={{ backgroundColor: 'rgba(255,255,255,0.72)' }}
                              ...
                              >
                                ...
                                <span className="text-[18px] tracking-[-0.02em]">{item.variant}</span>
                                <span className="text-[18px] tracking-[-0.02em] text-[color:var(--muted-ink)]">{item.genomicLocus}</span>
                                <Badge variant={item.classificationVariant} className="w-fit text-sm tracking-[0.08em]">
                                  {item.classification}
                                </Badge>
                              </motion.div>

                              <motion.div whileHover={{ y: -2 }} className="space-y-3 py-5 lg:hidden">
                                <div className="flex items-center justify-between gap-4">
                                  <span className="text-[24px] font-semibold tracking-[-0.04em] text-[color:var(--teal)]">
                                    {item.gene}
                                  </span>
                                  <Badge variant={item.classificationVariant} className="w-fit text-xs tracking-[0.08em]">
                                    {item.classification}
                                  </Badge>
                                </div>
                                <p className="text-base font-medium">{item.variant}</p>
                                <p className="text-sm uppercase tracking-[0.12em] text-[color:var(--muted-ink)]">{item.genomicLocus}</p>
                              </motion.div>
                            </motion.div>
                          ))}
                        </AnimatePresence>
                    ...
                    <section className="mt-10 grid gap-6 border-t border-[color:var(--line)]/90 pt-8 xl:grid-cols-3 xl:gap-8">
                      <InlineNote
                        ...
                        value={activeSession.run.report_payload.clinical_integration || 'Awaiting clinician interpretation.'}
                      />
                      <InlineNote
                        label="Recommendations"
                        value={activeSession.run.report_payload.recommendations || 'No recommendation available.'}
                      />
                      <InlineNote
                        label="Limitations"
                        value={activeSession.run.report_payload.limitations || 'No limitations captured for this run.'}
                      />
                    </section>
                  </>
                ) : (
                  <section className="grid gap-8 xl:grid-cols-[minmax(0,1.3fr)_320px]">
                    <motion.div
                      whileHover={{ y: -2 }}
                      className="grid gap-6 border-y border-[color:var(--line)]/90 py-8"
                    >
                      <div>
                        <p className="text-[11px] font-semibold uppercase tracking-[0.22em] text-[color:var(--muted-ink)]">
                          Intake briefing
                        </p>
                        <p className="mt-4 max-w-4xl text-[17px] leading-8 text-[color:var(--muted-ink)] xl:text-[18px] xl:leading-9">
                          Choose a genomic report PDF and run the demo pipeline. The backend will upload the report, create a run, call the live evidence services, and then populate this review workspace with the completed result.
                        </p>
                      </div>

                      <div className="flex flex-col gap-4 sm:flex-row sm:items-center">
                        <Button variant="secondary" size="lg" className="justify-start rounded-[20px]" onClick={handleChooseFile}>
                          <Upload className="h-4 w-4" strokeWidth={2.1} />
                          {pendingFile ? pendingFile.name : 'Select report PDF'}
                        </Button>
                        <Button size="lg" className="rounded-[20px]" onClick={handleRunPipeline} disabled={!pendingFile || isWorking}>
                          {isWorking ? <LoaderCircle className="h-4 w-4 animate-spin" strokeWidth={2.1} /> : <Play className="h-4 w-4" strokeWidth={2.1} />}
                    ...
                    <div className="rounded-[24px] border border-[color:var(--line)]/80 bg-[color:var(--summary-bg)] px-5 py-5">
                      <p className="text-[11px] font-semibold uppercase tracking-[0.22em] text-[color:var(--muted-ink)]">
                        Selected input
                      </p>
                      <div className="mt-5 space-y-4 text-sm text-[color:var(--muted-ink)]">
                        <div>
                          <span className="block text-[11px] font-semibold uppercase tracking-[0.18em] text-[color:var(--muted-ink)]">
                            File
                          </span>
                        ...
                        <div>
                          ...
                          <span className="mt-2 block text-base text-[color:var(--ink)]">{reportKind}</span>
                        </div>
                      </div>
                    </div>
                  </section>
            ...
            <aside className="border-t border-[color:var(--line)] bg-[linear-gradient(180deg,rgba(236,243,244,0.76),rgba(229,238,240,0.96))] px-6 py-8 sm:px-8 xl:px-10 xl:py-10 2xl:border-l 2xl:border-t-0 2xl:border-[color:var(--line)]">
              <div className="2xl:sticky 2xl:top-28">
                <div className="rounded-[28px] bg-white/48 px-5 py-5 shadow-[inset_0_1px_0_rgba(255,255,255,0.7)] backdrop-blur-sm xl:px-6">
                  <div className="mb-6 flex items-center gap-4">
                    <TestTubeDiagonal className="h-5 w-5 text-[color:var(--muted-ink)]" strokeWidth={1.8} />
                    <h3 className="text-[24px] font-semibold uppercase tracking-[0.12em] text-[color:var(--muted-ink)] xl:text-[29px]">
                  ...
                  {activeSession ? (
                    <div className="divide-y divide-[color:var(--line)]/80">
                      {evidenceCards.map((card, index) => {
                        const isSelectedCard = selectedEvidenceCard?.id === card.id
                        return (
                          <motion.button
                            key={card.id}
                            type="button"
                            ...
                            className={cn(
                              ...
                              isSelectedCard ? 'text-[color:var(--ink)]' : 'text-[color:var(--muted-ink)]',
                            )}
                          >
                            <div className="flex items-start justify-between gap-4">
                                ...
                                {card.warnings.length ? (
                                  <div className="mt-3 flex flex-wrap gap-2">
                                    {card.warnings.map((warning) => (
                                      <Badge key={warning} variant="warning" className="tracking-[0.12em]">
                              ...
                              <div className="shrink-0 text-right">
                                {card.badge ? <Badge variant={card.badgeVariant}>{card.badge}</Badge> : null}
                                {card.scoreLabel ? (
                                  <>
                                    ...
                                    {card.subLabel ? (
                                      <p className="mt-2 text-sm text-[color:var(--muted-ink)]">{card.subLabel}</p>
                                    ) : null}
                                  </>
                                ) : null}
                              </div>
                              ...
                              <div className="mt-6">
                                <div className="h-1.5 rounded-full bg-[color:var(--line)]">
                                  <motion.div
                                    ...
                                    initial={{ width: 0 }}
                                    animate={{ width: `${Math.max(card.progress * 100, 8)}%` }}
                                    transition={{ duration: 0.8, ease: 'easeOut', delay: 0.32 }}
                                    style={{ background: 'linear-gradient(90deg, var(--impact) 0%, #ca675f 100%)' }}
                                  />
                                </div>
                              </div>
                            ) : null}
                    ...
                    <div className="space-y-5">
                      {pipelineSteps.map((step, index) => {
                        const state = getPipelineStepState(step.id, pipelineMode)
                        return (
                          <motion.div
                            key={step.id}
                            initial={{ opacity: 0, y: 8 }}
                          ...
                          >
                            <div className={cn('mt-1 h-3.5 w-3.5 rounded-full border', pipelineDotClassName(state))} />
                            <div>
                              <p className="text-[11px] font-semibold uppercase tracking-[0.18em] text-[color:var(--muted-ink)]">
                                {step.title}
                              </p>
                              <p className="mt-3 text-[15px] leading-7 text-[color:var(--ink)]">{step.body}</p>
                            </div>
                          </motion.div>
                        )
                      })}
                ...
                <motion.section
                  ...
                  variants={surfaceReveal}
                  custom={0.22}
                  className="mt-8 rounded-[28px] bg-white/38 px-5 py-5 shadow-[inset_0_1px_0_rgba(255,255,255,0.7)] backdrop-blur-sm xl:px-6"
                >
                  <div className="mb-6 flex items-center gap-4">
                    <CircleAlert className="h-5 w-5 text-[color:var(--muted-ink)]" strokeWidth={1.8} />
                    <h3 className="text-[24px] font-semibold uppercase tracking-[0.12em] text-[color:var(--muted-ink)] xl:text-[29px]">
                      Clinician review
                    </h3>
                  </div>
                  ...
                  {activeSession ? (
                    <>
                      <Button
                        ...
                        onClick={openEditor}
                        disabled={isApproved || isDropped || actionMode !== 'idle'}
                      >
                        <span>Preview & Edit Report</span>
                      ...
                      <div className="mb-4 flex items-center justify-between gap-4">
                        ...
                        <div className="flex items-center gap-2">
                          ...
                          <Badge variant={reviewBadgeVariant(activeSession.run.review_status)}>
                      ...
                      <div className="mt-6 grid gap-4 sm:grid-cols-2">
                        <Button
                          ...
                          disabled={isApproved || isDropped || actionMode !== 'idle'}
                        >
                          {actionMode === 'dropping' ? <LoaderCircle className="h-5 w-5 animate-spin" /> : null}
                          Drop
                        </Button>
                        <Button
                          size="lg"
                        ...
                        >
                          {actionMode === 'approving' ? (
                            ...
                            <CheckCircle2 className="h-5 w-5 transition-transform duration-200 group-hover:rotate-6" strokeWidth={2.1} />
                          )}
                      ...
                      {(activeSession.run.warnings.length || selectedEvidenceCard?.warnings.length) && (
                    ...
                    <div className="space-y-4">
                      <p className="text-sm leading-7 text-[color:var(--muted-ink)]">
                        The clinician note field, approval controls, and approved PDF action appear here after a run completes.
                      </p>
                      <div className="rounded-2xl border border-dashed border-[color:var(--line-strong)] bg-white/24 px-4 py-4 text-sm text-[color:var(--muted-ink)]">
                        Review actions remain inactive until the backend returns a completed run payload.
                      </div>
                    </div>
                  )}
            ...
            <div className="grid max-h-[92vh] min-h-[760px] grid-cols-1 xl:grid-cols-[minmax(0,520px)_minmax(0,1fr)]">
              <div className="overflow-y-auto border-b border-[color:var(--line)] bg-white/70 px-6 py-6 xl:border-b-0 xl:border-r xl:px-7 xl:py-7">
                <DialogHeader className="pr-14">
                  ...
                  <DialogDescription>
                    Update the textual draft, save the run payload, and refresh the preview before approval freezes the report.
                  </DialogDescription>
                </DialogHeader>
                ...
                <div className="mt-6 rounded-[24px] border border-[color:var(--line)] bg-[color:var(--summary-bg)]/75 px-4 py-4">
                  <div className="grid gap-4 text-sm text-[color:var(--muted-ink)] sm:grid-cols-2">
                    <ReadOnlyMeta label="Case ID" value={formatCaseReference(activeSession.run.patient_id)} />
                    <ReadOnlyMeta label="Review state" value={formatReviewLabel(activeSession.run.review_status)} />
                  </div>
                </div>
                ...
                <div className="mt-6 space-y-5">
                  ...
                  <div>
                    ...
                    <Textarea
                      ...
                      onChange={(event) => updateDraftField('review_note', event.target.value)}
                  ...
                  <div className="rounded-[24px] border border-[color:var(--line)] bg-white/52 px-4 py-4">
                    ...
                    <div className="mt-3 space-y-3">
                      {variantRows.map((item) => (
                        <div key={`${item.gene}-${item.variant}`} className="rounded-2xl border border-[color:var(--line)] bg-white/70 px-4 py-3">
                          <p className="text-sm font-semibold text-[color:var(--ink)]">{item.gene}</p>
                          <p className="mt-1 text-sm text-[color:var(--muted-ink)]">{item.variant}</p>
                        </div>
                      ))}
                    </div>
                  </div>
                  ...
                  <div className="flex flex-wrap items-center justify-between gap-3 border-t border-[color:var(--line)] pt-5">
                    <p className="max-w-sm text-sm leading-6 text-[color:var(--muted-ink)]">
                      Saved edits update the run payload used by preview, approval, and final download.
                    </p>
                    <div className="flex gap-3">
                      <Button variant="ghost" onClick={() => setEditorOpen(false)} disabled={actionMode !== 'idle'}>
                        Close
                      </Button>
                      <Button variant="secondary" onClick={handleSaveDraft} disabled={actionMode !== 'idle'}>
                        {actionMode === 'saving_draft' ? <LoaderCircle className="h-4 w-4 animate-spin" /> : null}
                        Save Draft
                      </Button>
                    </div>
                  </div>
              ...
              <div className="flex min-h-[520px] flex-col bg-[color:var(--wash)]/75">
                <div className="flex items-center justify-between gap-4 border-b border-[color:var(--line)] bg-white/65 px-5 py-4 backdrop-blur-sm">
                  ...
                  <Button variant="ghost" onClick={() => setPreviewVersion(Date.now())} disabled={actionMode !== 'idle'}>
                      ...
                      >
                        <div className="flex h-full min-h-[600px] flex-col items-center justify-center gap-4 bg-white px-8 text-center">
                          <p className="max-w-sm text-sm text-[color:var(--muted-ink)]">
                            The inline PDF preview could not be rendered in this browser view.
                          </p>
                          <a
                            href={previewPdfUrl}
                            target="_blank"
        ...
        <AnimatePresence initial={false}>
          {chatOpen ? (
                ...
                <div className="mb-4 flex flex-wrap gap-2">
                  {starterPrompts.map((prompt) => (
                    <button
                      ...
                      className="rounded-full border border-white/10 bg-white/6 px-3 py-2 text-xs font-medium text-white/78 transition-all hover:-translate-y-0.5 hover:bg-white/10 hover:text-white"
                    >
                      {prompt}
                    </button>
                  ))}
            ...
            >
              ...
              >
                <MessageSquareMore className="h-4.5 w-4.5 text-[color:var(--chat-accent)]" strokeWidth={2.1} />
                Talk to report
              </motion.button>
            </motion.div>
          )}
...
function ActionIcon({
  ...
  return (
    <motion.button
      whileHover={disabled ? undefined : { y: -2, backgroundColor: 'rgba(255,255,255,0.84)' }}
      whileTap={disabled ? undefined : { scale: 0.97 }}
      className={cn(
        'rounded-2xl border border-transparent p-3 transition-colors hover:border-[color:var(--line)]',
      ...
      onClick={onClick}
    >
      <Icon className="h-6 w-6 xl:h-7 xl:w-7" strokeWidth={1.9} />
    </motion.button>
  )
}
...
function Detail({ label, value }: { label: string; value: string }) {
  return (
    <motion.div whileHover={{ y: -2 }}>
      <p className="text-[11px] font-semibold uppercase tracking-[0.18em] text-[color:var(--muted-ink)]">{label}</p>
      <p className="mt-3 text-[22px] font-semibold tracking-[-0.04em] sm:text-[26px] xl:mt-4 xl:text-[30px]">{value}</p>
    </motion.div>
  )
}
...
function InlineNote({ label, value }: { label: string; value: string }) {
  return (
    <div>
      <p className="text-[11px] font-semibold uppercase tracking-[0.18em] text-[color:var(--muted-ink)]">{label}</p>
      <p className="mt-3 text-[15px] leading-7 text-[color:var(--muted-ink)]">{value}</p>
    </div>
  )
}
...
function ReadOnlyMeta({ label, value }: { label: string; value: string }) {
  return (
    <div>
      <p className="text-[11px] font-semibold uppercase tracking-[0.18em] text-[color:var(--muted-ink)]">{label}</p>
      <p className="mt-2 text-base font-medium text-[color:var(--ink)]">{value}</p>
    </div>
  )
}
...
function renderSummary(summary: string, highlight: string | null) {
  ...
  return (
    <>
      {before}
      <span className="font-semibold text-[color:var(--teal)]">{highlight}</span>
      {after}
    </>
  )
}
...
function buildVariantRows(session: SessionRun) {
  ...
  return rows.map((row) => ({
    ...
    variant: [row.transcript_hgvs, row.protein_change].filter(Boolean).join(' / ') || row.consequence || 'No variant string available',
...
function buildEvidenceCards(evidence: EvidenceSourceSummary[]): EvidenceCard[] {
  return evidence.map((item) => {
    ...
    const badgeVariant = item.status === 'live' ? 'success' : item.status === 'fallback' ? 'warning' : 'default'

    if (item.source === 'clinvar') {
      const classification = String(item.summary.classification || 'Unclassified')
      const reviewStatus = String(item.summary.review_status || '')
      const conditions = Array.isArray(item.summary.conditions) ? item.summary.conditions.join(', ') : ''
      return {
        id: item.source,
        title,
    ...
    if (item.source === 'vep') {
      const mostSevereConsequence = prettifyConsequence(String(item.summary.most_severe_consequence || 'Not stated'))
      const distribution = item.summary.consequence_distribution as Record<string, number> | undefined
      const highestValue = distribution ? Math.max(...Object.values(distribution)) / 100 : 0.5
      return {
        id: item.source,
        title,
        badge: item.status,
    ...
    if (item.source === 'franklin') {
      const functional = String(item.summary.functional_data || 'Not available')
      const population = String(item.summary.population_data || 'Not available')
      const inSilico = String(item.summary.in_silico_prediction || 'Not available')
      return {
        id: item.source,
        title,
...
function pipelineDotClassName(state: 'idle' | 'active' | 'complete' | 'attention') {
  switch (state) {
    case 'active':
