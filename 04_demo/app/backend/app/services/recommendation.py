...
class RecommendationService:
    ...
    def apply_review(self, run_id: str, payload: ClinicianReviewPayload) -> ReviewResult:
        ...
        if run.review_status.value == 'dropped':
            raise HTTPException(status_code=status.HTTP_409_CONFLICT, detail='Dropped run cannot be reviewed.')

        note = (payload.review_note or '').strip()
        if not note:
            raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail='review_note is required.')
