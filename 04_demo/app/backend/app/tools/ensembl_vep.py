...
class EnsemblVepTool(FixtureBackedTool):
    ...
    def _fetch_live(self) -> ToolResult:
        ...
        canonical = next(
            ...
            if item.get('gene_symbol') == 'RPE65' and item.get('cds_start') == 260 and item.get('protein_start') == 87
        )
        term_counter = Counter(term for item in consequences for term in item.get('consequence_terms', []))
        total_terms = sum(term_counter.values()) or 1
        distribution = {key: round((count / total_terms) * 100, 1) for key, count in term_counter.items()}
        summary = {
