...
class FranklinTool(FixtureBackedTool):
    ...
    def _fetch_live(self) -> ToolResult:
        ...
        summary = {
            ...
            "functional_data": search_payload.get("classification", {}).get(
                "acmg_classification", "Unavailable"
            )
            if isinstance(search_payload, dict)
            else "Unavailable",
            "population_data": search_payload.get("annotations", {})
            .get("frequencies", {})
            .get("aggregated_frequency", "Unavailable")
            if isinstance(search_payload, dict)
            else "Unavailable",
            "in_silico_prediction": search_payload.get("annotations", {})
            .get("predictions", {})
            .get("aggregated_predictions", "Unavailable")
            if isinstance(search_payload, dict)
            else "Unavailable",
        }
        return ToolResult(
